...
::::: columns
::: {.column width="50%"}
![Typical Ramachandran plot made by the Procheck program, here shown on [Wikipedia](https://en.wikipedia.org/wiki/Ramachandran_plot#/media/File:1axc_PCNA_ProCheck_Rama.jpg).](images/rama_wiki.jpg){fig-align="center" width="375"}
:::

::: {.column width="50%"}
![Home-made Ramachandran plot created from the top2018 dataset by the Richardson Lab at Duke University. These are high-quality residues from high-quality, low redundancy protein chains in the PDB.](images/base_ramanchadran.png){fig-align="center" width="375"}
:::
:::::
::::::
...
-   [Journals]{.highlight} increasingly request [data deposition]{.highlight}
...
[“As open as possible, as closed as necessary”]{.highlight}:

Results and data may be kept closed if making them public in open access is against the researcher’s legitimate interests, e.g.
...
-   Publish in [Open Access journals]{.highlight}

    -   or make your preprint and/or accepter manuscript available

-   Share your data in [open repositories]{.highlight}
...
-   [Free and with good visibility]{.highlight} for well-known repositories ([BioRχiv](https://www.biorxiv.org/){.uri}, [ChemRxiv](https://chemrxiv.org/engage/chemrxiv/public-dashboard))
...
-   Possible to [revise the manuscript]{.highlight} and thus to upload accepted manuscript

    -   See [Google Scholar]

-   Possible to [link final paper]{.highlight} to preprint

-   Authors [retain copyright]{.highlight}
...
-   [Self-archiving]{.highlight} on repositories or personal sites

-   Uses [pre-prints or post-prints]{.highlight} (not final publisher version)

-   May have embargo periods

-   [Free access; no APCs]{.highlight}

## 💛 **Gold OA**

-   Published in [fully OA journals]{.highlight}

-   Immediate access to final [published version]{.highlight}

-   [APCs]{.highlight} usually required

-   [High visibility]{.highlight} and wide dissemination

## 💠 **Diamond OA**

-   [No APCs for authors or cost for readers]{.highlight}

-   Often funded by institutions, societies, governments

-   Focuses on providing free access to both readers and authors

-   [Free and equitable access for all]{.highlight}

## 🙅 **Hybrid**

-   Subscription journals with [optional OA per article]{.highlight}

-   [Authors pay APCs for OA]{.highlight}

-   Meets OA mandates

-   ["Double-dipping"]{.highlight}: [subscriptions **and** APCs]{.rn rn-type="crossed-off" rn-strokeWidth="3" rn-color="tomato"}

::: {.callout-caution collapse="true"}
## INSERM's opinion [\[source\]](https://www.inserm.fr/nos-recherches/science-ouverte/#gare-au-modele-hybride)

Certaines revues fonctionnent selon un modèle hybride : les articles y sont a priori publiés selon le modèle classique (payant pour le lecteur), mais la revue donne le choix aux auteurs de payer pour que leur travail soit publié en libre accès (payant pour l’auteur). [Publier en *Open Access* dans ce type de revue est dès lors fortement déconseillé car cela entraîne un double paiement]{.highlight} : celui de l’abonnement pour consulter l’ensemble de la revue et celui des frais de publication en *Open Access*
:::
::::
...
The license [impacts what people can do with your work]{.highlight}, e.g.

-   [CC BY]{.highlight}: Reusers can distribute
...
-   [CC BY-NC-ND]{.highlight}: Reusers can copy and distribute the work
...
The license [impacts the APCs]{.highlight}
...
The Couperin consortium [negotiates reading and publishing rights]{.highlight} on behalf of our trustees
...
1.  Select a publisher on Couperin.org
2.  Consult the publication rights
    ...
    2.  When several options are available, choose the one that fits your needs:
        ...
        2.  License
        3.  Cost
        4.  Institutional/funding requirements
    3.  Check institution eligibility, as shown on page + Excel file ([example for Springer Nature](https://www.couperin.org/wp-content/uploads/2024/07/Couperin_Publishing_liste_2025.xlsx))
...
4.  During submission, select the option you chose at step 2.2. It may be automatic (based on your institutional email/declared affiliation)
5.  Profit
...
-   F1. (Meta)data are assigned a globally [unique and persistent identifier]{.highlight}

-   F2. Data are described with [rich metadata]{.highlight} (defined by R1)
...
-   A1. (Meta)data are **r**[etrievable by their identifier]{.highlight} using a standardised communications protocol

    -   A1.1 The protocol is [open, free, and universally implementable]{.highlight}
...
-   I1. (Meta)data use a [formal, accessible, shared, and broadly applicable language]{.highlight} for knowledge representation.
...
-   R1. (Meta)data are richly described with a plurality of [accurate and relevant attributes]{.highlight}

    -   R1.1. (Meta)data are released with a clear and accessible data usage license

    -   R1.2. (Meta)data are associated with [detailed provenance]{.highlight}

    -   R1.3. (Meta)data meet domain-relevant [community standards]{.highlight}
:::
...
-   Facilitates [collaborations]{.highlight}
-   Increase [visibility and impact]{.highlight}
...
You probably know those of your community: make use of them and follow community standards.

## Zenodo

-   [General purpose open repository]{.highlight} built and operated by CERN and [OpenAIRE](https://www.openaire.eu/)
...
-   Why use Zenodo?
    ...
    -   [Safe]{.highlight}: Storage in CERN’s Data Centre for as long as CERN exists
    -   [Citeable]{.highlight}: Every upload is assigned a DOI
    -   No waiting time: Uploads available online immediately and DOI registered within seconds
    -   [Open or closed]{.highlight}: Share e.g. anonymized clinical trial data with only medical professionals via [restricted access mode](https://help.zenodo.org/docs/share/user-sharing/)
    -   [Versioning]{.highlight}: Dataset/software can be updated. DOI for all and specific versions
...
Platforms like [GitHub](https://github.com/) and [GitLab](https://gitlab.com/) offer numerous benefits not only to developers but also to researchers:

-   [Back-up]{.highlight} and [Version Contro]{.highlight}l: Reliable preservation and retrieval of code; track changes
-   [Collaboration]{.highlight}: Enable multiple contributors to work simultaneously
-   [Accessibility]{.highlight}: Increase visibility and attract contributors outside of the lab
-   [Documentation]{.highlight}: Provide comprehensive documentation and transparency
-   [Integration]{.highlight}: Automate workflows and integrates with other platforms (Zenodo, HAL)
-   [Open Science]{.highlight}: Support reproducibility and validation of research
-   [Licensing]{.highlight}: Apply open-source licenses easily
-   [Learning]{.highlight}: Explore projects and improve coding skills
...
[https://entrepot.recherche.data.gouv.fr/](entrepot.recherche.data.gouv.fr) is a national repository (Login with institutional email/orcid) with a [generic space](https://entrepot.recherche.data.gouv.fr/dataverse/espacegenerique), and institutional spaces:
...
-   [Couperin consortium](https://www.couperin.org/) 🇫🇷: Negociates on behalf of public entities for publication access/publishing costs (among other things)
    ...
    -   [Facilitateur d'accès aux ressources](https://www.couperin.org/services-et-activites/comparatifs/outils-facilitant-lacces-a-la-documentation-electronique/)
